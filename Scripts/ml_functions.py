...
def BPNN_1_DO(hidden_neuron,n_hidden,optim,a_func,per):
    '''A modified version of our original BPNN tested in our work.
    Arguments:
    ...
    Returns:
    modelexp (keras sequential model):  for training and predicting.
    '''
    modelexp = Sequential()
    modelexp.add(Dropout(per,input_shape=(500,1)))
    modelexp.add(Dense(50, input_shape=(500, 1),activation=None))
    for i in range(n_hidden):
        modelexp.add(Dense(hidden_neuron,activation=activations.tanh))
    modelexp.add(Dense(1))
...
def run_all_models(train,vali,test):
    ...
    BP_NN_1_PRED = BP_NN_1.predict(series_data_gen(test,1),steps=len(test)) # takes <5 mins
    
    #BP_NN_2 = BPNN_2(10,10,'Adamax')
    #BP_NN_2.summary()
    #BP_02_hist=BP_NN_2.fit_generator(series_data_gen(train,2),steps_per_epoch=100,epochs=int(len(train)/100))
    ...
    BP_NN_33.summary()
    BP_33_hist=BP_NN_33.fit_generator(series_data_gen(train,33),steps_per_epoch=100,epochs=int(len(train)/100))
    ...
    CV_NN_1.summary()
    CV_NN_1_hist = CV_NN_1.fit_generator(series_data_gen(train,1),steps_per_epoch=100,epochs=int(len(train)/100))
    ...
    #CV_NN_2.summary()
    #CV_NN_2_hist = CV_NN_2.fit_generator(series_data_gen(train,2),steps_per_epoch=100,epochs=int(len(train)/100))
    ...
    CV_NN_33.summary()
    CV_NN_33_hist = CV_NN_33.fit_generator(series_data_gen(train,33),steps_per_epoch=100,epochs=int(len(train/100))) 
    ...
    LS_TM_1.summary()
    LS_TM_1_hist = LS_TM_1.fit_generator(series_data_gen(train,1),steps_per_epoch=100,epochs=int(len(train/100))) 
    ...
    #LS_TM_2.summary()
    #LS_TM_2_hist = LS_TM_2.fit_generator(series_data_gen(train,2),steps_per_epoch=100,epochs=int(len(train/100))) 
    ...
    LS_TM_33.summary()
    LS_TM_33_hist = LS_TM_33.fit_generator(series_data_gen(train,33),steps_per_epoch=100,epochs=int(len(train)/100)) 
    ...
    models = [BP_NN_1, BP_NN_33,
              CV_NN_1, CV_NN_33,
              LS_TM_1, LS_TM_33]
    # all predictions on input test data
    predictions = [BP_NN_1_PRED, BP_NN_33_PRED,
                   CV_NN_1_PRED, CV_NN_33_PRED, 
                   LS_TM_1_PRED, LS_TM_33_PRED]
