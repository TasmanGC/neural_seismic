...
class trace:
    ...
    def plot_scalo2(self, title):
        ...
        cmap = plt.cm.PiYG
        plt.title("Scalogram")
        im = plt.imshow(values, interpolation=interpolation, cmap=cmap, aspect="auto",
    ...
    def plot_scalo(self, state):
        ...
        if state > 0:
            ...
            plt.imshow(values, interpolation=interpolation, cmap=cmap, aspect="auto",
                       origin="lower", extent=[0, 500, 0, len(values)])
            fig = f
    ...
    def gen_feat_space(self):
        """Generates a feature space for binary classification using DWT + RBF interpolation."""
        ...
        raw = np.asarray(self.Signal_1[0])
        feat_space["Raw"] = raw
        filt = np.asarray(self.Signal_1[1])
        ...
        for a in range(num_lev):
            ...
            extrap_rbf = np.interp([500], xnew, fi)
            X_val_2 = []
        ...
        if Rio_fb != None:
            ones = np.ones(((500 - Rio_fb),), dtype=int)
            zeros = np.zeros((Rio_fb,), dtype=int)
            method2 = np.append(zeros, ones)
            feat_space[column_headers[35]] = method2
        if Atlas_fb != None:
            ones = np.ones(((500 - Atlas_fb),), dtype=int)
            zeros = np.zeros((Atlas_fb,), dtype=int)
            method2 = np.append(zeros, ones)
            feat_space[column_headers[36]] = method2

        feat_space = (feat_space - feat_space.min()) / (feat_space.max() - feat_space.min())
        self.feat_space = feat_space.astype(np.float32)
